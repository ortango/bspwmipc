...
impl BspwmFuncs for UnixStream {
    fn send_receive_bspwm_message(&mut self, message: &str) -> String {
        ...
        self.read_to_string(&mut response)
            .expect("failed to communicate with bspwm.");
        response
    }
}
...
impl BspwmConnection {
    ...
    pub fn get_monitor(&mut self, id: &u32) -> Monitor {
        let reply = self
            .stream
            .send_receive_bspwm_message(&("query -T -m ".to_owned() + &id.to_string()));
        serde_json::from_str(&reply).unwrap()
    }
    pub fn get_desktop(&mut self, id: &u32) -> Desktop {
        let reply = self
            .stream
            .send_receive_bspwm_message(&("query -T -d ".to_owned() + &id.to_string()));
        serde_json::from_str(&reply).unwrap()
    }
    pub fn get_node(&mut self, id: &u32) -> Node {
        let reply = self
            .stream
            .send_receive_bspwm_message(&("query -T -n ".to_owned() + &id.to_string()));
        serde_json::from_str(&reply).unwrap()
    }
}
